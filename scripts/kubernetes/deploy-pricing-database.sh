...
echo
echo "Applying Pricing PostgreSQL Services..."

kubectl apply \
  -f "${SERVICE_MANIFEST}"

echo
echo "Applying Pricing PostgreSQL StatefulSet..."
...
kubectl rollout status \
  statefulset/pricing-service-db \
  --namespace "${NAMESPACE}" \
  --timeout "${ROLLOUT_TIMEOUT}"

echo
echo "Waiting for Pricing PostgreSQL Pod readiness..."

kubectl wait \
  --for=condition=Ready \
  pod \
  --selector='app.kubernetes.io/name=pricing-service-db,app.kubernetes.io/component=database' \
  --namespace "${NAMESPACE}" \
  --timeout "${ROLLOUT_TIMEOUT}"

echo
echo "Pricing PostgreSQL resources:"

kubectl get \
  statefulset,pod,service,persistentvolumeclaim \
  --namespace "${NAMESPACE}" \
  --selector='app.kubernetes.io/name=pricing-service-db' \
  -o wide

echo
echo "Pricing PostgreSQL was deployed successfully."